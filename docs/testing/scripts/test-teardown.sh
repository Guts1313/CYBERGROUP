#!/usr/bin/env bash
# M4 #30 — restore the realm to production-like config after a test pass.
#   - disables directAccessGrantsEnabled on iam-frontend
#   - re-adds CONFIGURE_TOTP required action on every demo user
set -euo pipefail
...
echo "=== disable directAccessGrantsEnabled on iam-frontend ==="
...
docker exec "$KC" /opt/keycloak/bin/kcadm.sh update "clients/$CLIENT_UUID" \
    -r "$REALM" -s directAccessGrantsEnabled=false

echo "=== re-add CONFIGURE_TOTP required action on demo users ==="
for u in admin-demo itmgr-demo hrmgr-demo dev-demo normal-demo; do
    ...
    docker exec "$KC" /opt/keycloak/bin/kcadm.sh update "users/$UID2" \
        -r "$REALM" -s 'requiredActions=["CONFIGURE_TOTP"]'
    echo "  $u → restored (uuid=$UID2)"
done
